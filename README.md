...
- [Sudanese Relocation Recommender 🌍](#sudanese-relocation-recommender-)
  ...
  - [Future Work and Roadmap](#future-work-and-roadmap)
    - [Short Term (1 to 3 months)](#short-term-1-to-3-months)
    - [Medium Term (3 to 6 months)](#medium-term-3-to-6-months)
    - [Long Term (6 to 12 months)](#long-term-6-to-12-months)
...
This project is actively evolving. Below are planned improvements and research directions that will help increase accuracy, usability, and humanitarian relevance.

### Short Term (1 to 3 months)

1. **Increase survey sample size**  
   Promote the form through community networks, WhatsApp groups, Sudanese diaspora pages, and humanitarian channels to collect a larger and more representative dataset.

2. **Add more destination features**  
   Expand the country features dataset with real cost of living indexes, visa accessibility data, healthcare access levels, and diaspora numbers.

3. **Implement clustering evaluation metrics**  
   Use silhouette score and Davies Bouldin index to refine k means clustering for both users and destination countries.

4. **Baseline model comparisons**  
   Evaluate weighted scoring against logistic regression, random forest, and gradient boosted trees.

5. **Model explainability module**  
   Add a simple explanation function that highlights the top features contributing to each recommendation.

### Medium Term (3 to 6 months)

1. **Neural network scoring model**  
   Train and evaluate a feed forward neural network that outputs a suitability score for each (user, destination) pair.

2. **K fold cross validation**  
   Implement k fold cross validation for model selection and benchmarking across algorithms.

3. **User interface prototype**  
   Build a simple Streamlit or Gradio application for user input and real time recommendations. This includes a clean UI for selecting preferences and viewing recommended destinations.

4. **Model documentation and versioning**  
   Add model cards, version tags, and performance summaries in the `models/` folder.

5. **Automated data pipeline**  
   Create scripts in `src/data_processing/` that automatically clean survey responses and update country features from selected APIs or datasets.

### Long Term (6 to 12 months)

1. **Dynamic data updates**  
   Integrate continuous updates for country data such as visa policy changes, cost of living shifts, or conflict related alerts using web scraping or periodic dataset refresh.

2. **Location level recommendations (city ranking)**  
   Move beyond country ranking by adding major cities within each destination and scoring them independently.

3. **Support need matching**  
   Link recommendations to resources such as job platforms, housing programs, scholarship funding, and legal assistance networks based on the support needs question.

4. **Partnership development**  
   Collaborate with NGOs, refugee assistance organizations, or diaspora networks to evaluate outputs, gather feedback, and assess practical deployment uses.

5. **Ethical review and safety testing**  
   Prior to any real world use, conduct an ethical risk assessment to prevent potential harm, misinformation, or bias.

### Planned Deliverables

- Survey dataset in anonymized form  
- Country features CSV file  
- Notebooks for clustering, modeling, and evaluation  
- Mixed scoring recommender function  
- Working demo application  
- Documentation and ethical statement  
