...
For detailed setup and usage instructions (CLI + Streamlit), see:

➡️ [HOW_TO_USE.md](docs/HOW_TO_USE.md)

In short:

- Use the **CLI** to test recommendations for existing survey respondents.
- Use the **Streamlit app** to enter a new profile and get personalized suggestions.
...
- ✅ Build an interactive Streamlit form so users can input their own profile
- 🔜 Add additional fields (passport status, family size, medical constraints)
- 🔜 Integrate live visa / policy data from external APIs
- 🔜 Deploy to NGO partners or community organizations for feedback
